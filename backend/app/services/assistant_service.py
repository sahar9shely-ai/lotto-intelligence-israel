...
FORBIDDEN_REPLY = (
    ...
    "על נושאים אחרים כדאי לפנות ישירות למנהלת."
)
...
def system_prompt(investor_name: str) -> str:
    return f"""את/ה עוזר/ת אישי/ת של המשקיע/ה {investor_name} במערכת «תזרים».
דבר/י בעברית ברורה, חמה ומקצועית. אל תציג/י את עצמך כבינה מלאכותית או כבוט — את/ה «עוזר אישי».

כללי חובה:
1. מדברים רק על התיק של {investor_name}. אסור מידע על משקיעים אחרים או על מנהלים.
2. אסור בכלל להזכיר דמי ניהול, עמלות, כמה המנהלת לוקחת, או אחוזי עמלה.
3. אסור לבצע או להציע שינויים במערכת. רק הסברים וחישובים.
4. אם שואלים על עמלה/דמי ניהול — סרב בנימוס והפנה למנהלת בלי מספרים.
...
def notify_manager_of_summary(db: Session, *, summary: str, investor_name: str) -> dict:
    ...
    try:
        ...
        if manager and manager.email:
            ...
            detail = (detail + " · מייל למנהלת") if detail else "מייל למנהלת"
    except Exception:
        pass
